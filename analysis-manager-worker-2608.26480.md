...
4. La ganancia del paper es **mayor justo en el régimen de Trimegisto**: modelos locales pequeños/medianos con gestión de contexto débil (Qwen3.8-27B +23.4 puntos es el resultado estrella). Y el coste marginal de tokens con modelos locales (electricidad) hace que el "triple del bill" del paper importe mucho menos.
...
Benchmark: LiveCodeBench release_v6, 100 problemas hard más recientes. Nueve modelos (Qwen3.5-9B, Qwen3.6-35B-A3B, Qwen3.8-27B, Minimax-M3, Kimi-K3, Opus-5, GPT-5.6-Terra, GPT-5.6-Luna, Claude Fable 5).
...
1. **Gestión de contexto / memoria externa.** El estado vive en disco (plan/tasks/notes), no en un transcript creciente. Cada worker ve una vista compacta y curada. Esto reduce longitud por llamada, evita truncamiento y evita el colapso de razonamiento: Qwen3.8-27B single repitió una línea 7.743 veces y gastó 250k tokens sin emitir código; el manager lo evita porque cada llamada corta "llega al disco".
...
- Que la ganancia es **mayor en modelos locales con gestión de contexto débil** — el régimen exacto de los tiers locales de Trimegisto (el mismo Qwen3.8-27B que aparece en los ejemplos de tiers de llama-infra es el modelo con Δ +23.4).
