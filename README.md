...
Model ids are humanized — last path segment, separators to spaces, version prefixes kept lower-case (`deepseek/deepseek-v4-flash` → `Deepseek v4 Flash`, `moonshot/kimi-k3` → `Kimi K3`, `.../Example-27B-ROCmFP4-FAST.gguf` → `Example 27B ROCmFP4 FAST`). Long labels are capped with `…`. Before a worker's first response the label falls back to the requested model.
